...
1. `convert-linux-amd64.zip`をダウンロード
2. ZIPファイルを展開
3. ターミナルで展開したディレクトリに移動し、以下を実行：
...
- **C/C++コンパイラ**: `goheif`はHEVCデコーダ（libde265）のソースを内蔵しているため、システムの`libheif`ライブラリは不要です。CGOのビルドに必要なコンパイラのみ用意してください
  - **Linux**: `build-essential`（gcc/g++）
  - **macOS**: Xcode Command Line Tools（`xcode-select --install`）
  - **Windows**: MinGW-w64（例: MSYS2の`mingw-w64-x86_64-gcc`パッケージ）
